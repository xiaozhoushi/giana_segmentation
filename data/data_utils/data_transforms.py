...
class RandomDownResizeUpResize(torch.nn.Module):
    ...
    def __init__(self, scale, interpolation=None):
        super().__init__()
        if not isinstance(scale, (float, Sequence)):
            raise TypeError("Size should be float or sequence. Got {}".format(type(size)))
    ...
    def forward(self, img):
        ...
        if self.interpolation is not None:
            random_interpolation = self.interpolation
        else:
            index = random.randint(0, len(self.inter_method)-1) 
            random_interpolation = self.inter_method[index]
        img = F.resize(img, (int(h * random_scale), int(w * random_scale)), random_interpolation)
        return F.resize(img, (h, w), random_interpolation)
...
class RandomAffine(transforms.RandomAffine):
    ...
    def forward(self, img1, img2):
        ...
        return F.affine(img1, *ret, fill=self.fill), \
            F.affine(img2, *ret, fill=self.fill)
...
class RandomResizedCrop(transforms.RandomResizedCrop):
    ...
    def forward(self, img1, img2):
        ...
        return F.resized_crop(img1, i, j, h, w, self.size, self.interpolation), F.resized_crop(img2, i, j, h, w, self.size, InterpolationMode.NEAREST) 
...
class RandomChoiceOrder(object):
...
class RandomChoiceOrderImgMask(object):
    ...
    def __call__(self, img, mask):
        ...
        random.shuffle(order)
        for i in order:
            img, mask = transform_list[i](img,mask)
        return img, mask
